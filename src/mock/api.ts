...
export const addTodo = (title: string): Promise<Todo> => {
    return new Promise((resolve) => {
        setTimeout(() => {
            const newTodo: Todo = { id: Date.now(), title: title, description: "", status: TodoStatus.Pending };
            todos.push(newTodo);
            console.log(todos);
            resolve(newTodo);
        }, 200);
...
export const updateTodoStatus = (id: number, newStatus: TodoStatus): Promise<Todo | undefined> => {
    return new Promise((resolve) => {
        setTimeout(() => {
